...
# # Overview
# This script processes the whole dataset with TOP-5 SUCCESS RATE CALCULATION
# Extends predict_pl_test_top5.py to work on all protein-ligand pairs
...
save_rdkit_path = "./PL_results_top5/PL_rdkit/"
os.system(f"mkdir -p {save_rdkit_path}")

# Configuration: Number of pose generation repeats
# Higher = better pose quality (more poses to choose from), but slower
# n_repeat=1: fast, single pose per prediction (~9 seconds per pose)
# Number of poses to generate per binding site
# For top-5 success rate, we need many poses to rank from
# n_repeat=20: generates 20 poses per binding site (good for top-5)
# n_repeat=50: generates 50 poses per binding site (more thorough)
N_REPEAT = 5  # Generate this many poses per binding site for ranking
...
save_protein_results_path = "./PL_results_top5/protein_list/"
...
dataset_path = f"./PL_results_top5/dataset/"
...
info.to_csv(f"./PL_results_top5/info_with_predicted_affinity.csv")
...
# # WITH TOP-5 SUCCESS RATE CALCULATION
...
result_folder = f'./PL_results_top5/result/'
    ...
    if compound_name in ligand_file_paths:
        ...
        try:
            ...
            if native_mol is not None:
                ...
                native_coords = torch.tensor([list(native_conf.GetAtomPosition(i)) for i in range(native_mol.GetNumAtoms())], dtype=torch.float32).to(device)
                logging.info(f"  Loaded native ligand coordinates from: {native_ligand_file}")
                logging.info(f"  Native ligand has {native_mol.GetNumAtoms()} atoms")
...
if len(valid_rmsd_results) > 0:
    ...
    rmsd_df.to_csv(f"./PL_results_top5/top5_rmsd_results.csv", index=False)
    logging.info(f"RMSD results saved to: ./PL_results_top5/top5_rmsd_results.csv")
else:
